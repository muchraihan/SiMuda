    ...
    @auth
        ...
        <nav class="flex-grow p-4 space-y-1.5 overflow-y-auto no-scrollbar">
            ...
            @if (Auth::user()->peran === 'siswa')
                ...
                <a href="{{ route('siswa.riwayatpeminjaman.index') }}" class="flex items-center px-4 py-3 rounded-xl text-sm font-medium transition-all duration-200 group {{ request()->routeIs('siswa.riwayatpeminjaman.*') ? 'bg-green-600 text-white shadow-lg shadow-green-900/20' : 'text-gray-400 hover:bg-white/5 hover:text-green-400' }}">
                    <svg class="w-5 h-5 mr-3 transition-transform group-hover:scale-110 {{ request()->routeIs('siswa.riwayatpeminjaman.*') ? 'text-white' : 'text-gray-500 group-hover:text-green-400' }}" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    Riwayat Saya
                </a>
